...
# Parolen Header (p-.. & pdev-...-..) extrahieren als Vorbereitung für Lookup
voting_parties_combined <- bind_rows(
  
  # Block 1: Staatsebene
  tibble(voting_parties = grep("^p-",                # Mutterparteien / National
                               names(voting_5y),
                               value = TRUE)) %>%
    mutate(source = "state") %>%
    separate(
  ...
  # Block 2: Kantonsebene, Jungparteien, Frauen etc.
  tibble(voting_parties = grep("^pdev-",             # Abweichende Sektionen
                               names(voting_5y),
                               value = TRUE)) %>%
    mutate(source = "canton") %>%
    separate(
      voting_parties,
      into = c("prefix", "short_name", "add_info"),
      sep = "[-_]",
...
#TODO: in Lookup Liste aufnehmen

lookup_elec_nr_sr_combined_canton <- elec_nr_sr_combined %>% 
  select(CantonAbbreviation, CantonName) %>% 
  distinct()

lookup_elec_nr_sr_combined_parties <- elec_nr_sr_combined %>% 
  select(PartyAbbreviation, PartyName) %>% 
  distinct()



# Parteinamen und Abkürzungen vereinheitlichen
