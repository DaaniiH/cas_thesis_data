...
Wahlberechtigte_1990_2024 <- read_excel("data/Wahlberechtigte_1990-2024.xlsx",
  ...
  rename(year = Jahr,
         stimmberechtigte_avg = Stimmberechtigte_avg)
...
# value als Character
voting_long$value <- as.character(voting_long$value)
...
#### Referenztabellen - Erläuterungen gem. Codebook

##### Funktion zur Erstellung der Tabellen

`create_reference_tables()`

Kreiert in sepratem Environment Referenztabellen für Spalten deren Inhalte..

-   einstellige Zahlen und/oder

-   Präfixe (gemäss Selektion in Funktionsaufruf)

beinhalten, sofern diese nicht bereits bestehen und editiert wurden.

```{r}

create_reference_tables <- function(df,
                                    exclude_col = NULL,
                                    ref_env = .GlobalEnv,
                                    column_prefix = NULL) {
  library(dplyr)
  ...
  # Werte 0-9
  is_single_digit_numeric <- function(x) {
    vals <- unique(na.omit(x))
    all(vals %in% 0:9) || all(vals %in% as.character(0:9))
  }
  
  # Spaltennamen mit einstelligen Ziffern behalten
  cols_single_digit <- df %>%
    { if (!is.null(exclude_col)) select(., -all_of(exclude_col)) else . } %>%
    select(where(is_single_digit_numeric)) %>%
    names()
  
  
  # Spaltennamen, die nach Präfix immer berücksichtigt werden
  cols_force <- character(0)
  if (!is.null(column_prefix)) {
    # Alle Spalten, die mit einem der Präfixe beginnen
    pat <- paste0("^(",
                  paste(column_prefix,
                              collapse = "|"),
                  ")")
    cols_force <- names(df)[grepl(pat,
                                  names(df))]
    # Ausgeschlossene Spalten entfernen
    if (!is.null(exclude_col)) {
      cols_force <- setdiff(cols_force,
                            exclude_col)
    }
  }
  
  cols_all <- unique(c(cols_single_digit, cols_force))
 
  # Reporting-Listen
  created <- character()
  existing <- character()
 
  # Prüft, ob Referenztabelle existiert und label keine Dummy-Werte enthält (im ref_env)
  should_overwrite_ref <- function(ref_name, env) {
    if (!exists(ref_name,
                envir = env)) return(TRUE)
    ref_tbl <- get(ref_name,
                   envir = env)
    if ("label" %in% names(ref_tbl) && any(!grepl("dummy",
                                                  ref_tbl$label))) return(FALSE)
    return(TRUE)
  }
  
  # Referenztabellen ggf. erstellen und ins ref_env laden
  ref_tables <- map(cols_all, function(col) {
    ref_name <- paste0("ref_", col)
    if (should_overwrite_ref(ref_name, ref_env)) {
      created <<- c(created, ref_name)
      df %>%
        distinct(!!sym(col)) %>%
        arrange(!!sym(col)) %>%
        mutate(label = paste0(col, "_dummy", row_number()))
    } else {
      existing <<- c(existing, ref_name)
      get(ref_name, envir = ref_env)
    }
  })
  names(ref_tables) <- paste0("ref_", cols_all)
  list2env(ref_tables, ref_env)
  
  # Rückgabe als Liste für weitere Auswertung oder Ausgabe
  list(
    created = created,
    existing = existing,
    ref_names = names(ref_tables),
    ref_env = ref_env
  )
}
```

##### Funktion zur Bildung der Refernztabellen ausführen 

ggf. Wiederholen nach Bearbeitung um verbleibende anzuschauen.

```{r}

# Separates Environment für Referenztabellen anlegen
if (!exists("ref_env")) {
  ref_env <- new.env()
}

# Funktion ausführen / Speichern
result <- create_reference_tables(df = voting_5y_reduced_fields,
                                  exclude_col = "anr",
                                  ref_env = ref_env,
                                  column_prefix = c("p-",
                                                    "pdev-",
                                                    "-pos"))
# Kreierte, zum bearbeiten
result$created

# Existierende (falls Inhalt von "dummy"-Werte abweicht)
result$existing
...
Ggfs. Betrachten / Löschen

```{r}

# Einzelne abfragen
# get("ref_p-svp", envir = ref_env)
# get("ref_pdev-x", envir = ref_env)


# Einzelne Referenz-Tabellen wieder löschen (Eingabe via Konsole)
# ref_tables_to_remove <- scan(what = character())
# rm(list = to_remove, envir = .GlobalEnv)

```

#####  Referenz-Tabellen bearbeiten
...
# Präfix-Pattern für ref_p- und ref_pdev-
pattern <- "^ref_p(dev)?-"

# Label-Vektor 
labels_vec <- c(
  # Lookup für ref_p & ref_pdev-
  "1\tJa-Parole",
  "2\tNein-Parole",
  "3\tkeine Parole",
  "4\tEnthaltung",
  "5\tkeine Parole",
  "8\tGegenentwurf (Stichfrage))",
  "9\tVolksinitiative (Stichfrage)",
  "66\tkeine Parole",
  ".\tkeine Parole",
  "9999\tkeine Parole",
  "\tkeine Parole"
...
# Vektor Dataframe umwandeln
labels_df <- read.table(text = labels_vec,
  sep = "\t",
  col.names = c("wert", "label"),
  stringsAsFactors = FALSE,
  fill = TRUE,
  quote = ""
)

# Selektiere Namen, mit ref_p- oder ref_pdev- beginnend & KEIN 'others' enthalten
sel_names <- result$created[
  grepl(pattern,
        result$created) & !grepl("others",
                                 result$created)
]

for (obj_name in sel_names) {
  ref_tbl <- get(obj_name, envir = ref_env)
  value_col <- setdiff(names(ref_tbl), "label")  # Spaltenname mit den Werten

  # Werte als Character für den Join
  ref_tbl[[value_col]] <- as.character(ref_tbl[[value_col]])
  labels_df$wert <- as.character(labels_df$wert)

  # Join nach Wert-Spalte
  ref_tbl <- left_join(ref_tbl, labels_df, by = setNames("wert", value_col))

  # label-Spalte ersetzen
  ref_tbl$label <- ref_tbl$label.y
  ref_tbl$label.y <- NULL
  ref_tbl$label.x <- NULL

  assign(obj_name, ref_tbl, envir = ref_env)

  # Optional: Melde unbekannte Werte
  if (any(is.na(ref_tbl$label))) {
    cat("\n", obj_name, " (neue/unbekannte Werte):\n", sep = "")
    print(ref_tbl[is.na(ref_tbl$label), , drop = FALSE])
  }
}
```
...
labels_vec <- c(
  ...
  "2\tAblehnend",
  "3\tKeine",
  "8\tVorzug Gegenentwurf (bei Stichfragen)",
  "9\tVorzug Volksinitiative (bei Stichfragen)")
...
elec_kanton_lvl_state <- elec_all_combined_input %>%
  ...
  left_join(Wahlberechtigte_1990_2024,
            by = c("year",
                   "Kt")) %>%
  mutate(n_seats_weighted = n_seats * stimmberechtigte_avg) %>%
  group_by(year,
           Partei) %>%
  summarise(n_seats_sum = sum(n_seats,
                              na.rm = TRUE),
...
elec_gemeinde_lvl_gemeinde  <- elec_all_combined_input %>%
  ...
  group_by(year,
           Gemeinde) %>%
  mutate(n_seats_pct = n_seats_sum / sum(n_seats_sum,
                                         na.rm = TRUE)) %>%
  ungroup()
...
elec_gemeinde_lvl_kt <- elec_all_combined_input %>%
  ...
  group_by(year,
           Kt) %>%
  mutate(n_seats_pct =                                      # ungewichtet %
           n_seats_sum / sum(n_seats_sum,
                             na.rm = TRUE),
         n_seats_pct_weighted =                             # gewichtet %  
