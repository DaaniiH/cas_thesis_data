...
daten_long <- daten_wide %>%
  pivot_longer(
    cols = matches("(_Frauen|_Männer|_Total)$"),     # Spaltenauswahl/Daten
    names_to = c("Partei", "m_w_Total"),             # Neue Spalte für Label
    names_pattern = "^(.*)_(Frauen|Männer|Total)$",  # RegEx 
    values_to = "Wert")                               # Neue Spalte für Werte
...
# Alle Sheetnamen auslesen
all_sheets <- excel_sheets(dateipfad)

selected_sheets <- c(2024:2019)

# Für jedes Sheet das lange Dataframe erzeugen und in eine Liste speichern
daten_liste <- lapply(selected_sheets,
                      function(sheet) {
                        importiere_sheet(dateipfad, sheet)})

# Optional: Alle Dataframes zu einem großen Dataframe zusammenfügen
daten_gesamt <- bind_rows(daten_liste, .id = "Sheet")
```
