...
# 1. Datumsspalten finden
date_cols <- grep("^\\d{4}-\\d{2}-\\d{2}$", names(elec_nr_sr_combined), value = TRUE)

# 2. Gruppieren und Summen berechnen
result_sum <- elec_nr_sr_combined %>%
  group_by(PartyName) %>%
  summarise(across(all_of(date_cols), sum, na.rm = TRUE), .groups = "drop")

# 3. Prozentwerte berechnen und anfügen
result_pct <- result_sum %>%
  mutate(across(
    all_of(date_cols),
    ~ . / sum(.) * 100,
    .names = "{.col}_pct"
...
# 4. Spaltenreihenfolge anpassen: immer [Datum, Datum_pct, Datum2, Datum2_pct, ...]
new_order <- c(
  "PartyName",
  as.vector(rbind(date_cols, paste0(date_cols, "_pct")))
)

result_final <- result_pct %>%
  select(all_of(new_order))

print(result_final)


write_xlsx(result_final, "temp.xlsx")
```
