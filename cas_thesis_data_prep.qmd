...
# a) Zeitraum 2019–2025 (Variablen zu reduziere)
df_2019_2025 <- voting_5y_final %>%
  filter(year >= 2019 & year <= 2025)

# b) Zeitraum 2019-2023 (alle Variablen)
df_2019_2023 <- voting_5y_final %>%
  filter(year >= 2019 & year <= 2023)


# 3. Korrelation zwischen tatsächlichem Ergebnis und Parolen
...
# b) Visualisierung der Korrelation
...
#5. Lineare Regression: Welche Faktoren beeinflussen das Abstimmungsergebnis?
# Einfaches Modell mit Parolen
model1 <- lm(annahme ~ state_parole_Ja + kt_parole_Ja + gemeinde_parole_Ja, data = df_2019_2025)
summary(model1)

# Erweitertes Modell mit weiteren Variablen
model2 <- lm(annahme ~ state_parole_Ja + kt_parole_Ja + gemeinde_parole_Ja +
               dep + rechtsform + bet + inserate_total + inserate_ja +
               inserate_nein + inserate_neutral + mediares_tot + mediaton_tot,
             data = df_2019_2025)
summary(model2)


# 6. Analyse für Zeitraum 2019–2023 (mit leer, ungültig, gültig)
# Modell mit zusätzlichen Variablen
model3 <- lm(volkja_proz ~ state_parole_Ja + kt_parole_Ja + gemeinde_parole_Ja +
               dep + rechtsform + bet + inserate_total + inserate_ja +
               inserate_nein + inserate_neutral + mediares_tot + mediaton_tot +
               leer + ungultig + gultig,
             data = df_2019_2023)
summary(model3)
