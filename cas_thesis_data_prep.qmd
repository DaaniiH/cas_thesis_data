...
# wilcox.test(diff_state ~ rechtsform, data = df_sub)
wilcox.test(diff_kanton ~ rechtsform, data = df_sub)
# wilcox.test(diff_gemeinde ~ rechtsform, data = df_sub)


# Mediane
df_sub %>% group_by(rechtsform) %>% summarise(Median = median(diff_kanton))
```
...
# Tabelle 1: VARIANZ
df1 <- head(summary_stats_v1)
ft1 <- format_table(df1)

# Tabelle 2: VARIANZ Gruppiert nach Rechtsform
df2 <- head(summary_stats_v2)
ft2 <- format_table(df2)
...
#### Tables vergleichende Gruppenstatistik (Word)
