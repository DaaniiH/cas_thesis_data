...
library(DescTools)
library(dplyr)    # am Schluss laden, u.a. select() greift sonst aufs falsche
```
...
voting_5y_final <- voting_5y_final %>%
  mutate(
    diff_state = state_parole_Ja - volkja_proz,
    diff_kanton = kt_parole_Ja - volkja_proz,
    diff_gemeinde = gemeinde_parole_Ja - volkja_proz
  )
...
# Visualisierung nach "dep"-Gruppe (z.B. als Facet)
...
voting_5y_final %>%
  ...
  summarise(
    n = n(),
    p_value = if (n() >= 3) shapiro.test(diff_state)$p.value else NA
  )

# Boxplot zur visuellen Prüfung
ggplot(voting_5y_final, aes(x = rechtsform, y = diff_state)) +
  geom_boxplot() +
  theme_minimal() +
  labs(title = "Abweichung diff_state nach Rechtsform",
       x = "Rechtsform", y = "diff_state")

# Normalverteilung je Gruppe prüfen (Shapiro-Wilk)
voting_5y_final %>% 
  filter(rechtsform != "Obligatorisches Referendum") %>% 
  group_by(rechtsform) %>% 
  summarise(p_value = shapiro.test(diff_state)$p.value)


# Einfaktorielle ANOVA
anova_model <- aov(diff_state ~ rechtsform, data = voting_5y_final)
summary(anova_model)


#  Wenn Pr(>F) < 0.05, ist der Unterschied signifikant → Rechtsform beeinflusst diff_state.
...
ANOVA Test alle Ebenen

HIER NOCH WEITERARBEITEN / ABSCHLIESSSEN, DANN KORRELATION UND REGRESSION

```{r}
# 1. Gruppengröße prüfen
voting_5y_final %>%
  count(rechtsform)

# 2. Shapiro-Wilk-Test für alle 3 diff-Variablen
normality_tests <- voting_5y_final %>%
  group_by(rechtsform) %>%
  summarise(
    n = n(),
    p_diff_state = if (n >= 3) shapiro.test(diff_state)$p.value else NA,
    p_diff_kanton = if (n >= 3) shapiro.test(diff_kanton)$p.value else NA,
    p_diff_gemeinde = if (n >= 3) shapiro.test(diff_gemeinde)$p.value else NA
  )

print(normality_tests)

# 3. Boxplots zur visuellen Prüfung
for (var in c("diff_state", "diff_kanton", "diff_gemeinde")) {
  ggplot(voting_5y_final, aes(x = rechtsform, y = .data[[var]])) +
    geom_boxplot() +
    theme_minimal() +
    labs(
      title = paste("Abweichung", var, "nach Rechtsform"),
      x = "Rechtsform",
      y = var
    ) +
    print()
...
# 4. ANOVA + Tukey für alle 3 diff-Variablen
for (var in c("diff_state", "diff_kanton", "diff_gemeinde")) {
  cat("\n==============================\n")
  cat("ANOVA für:", var, "\n")
  cat("==============================\n")
  
  formula <- as.formula(paste(var, "~ rechtsform"))
  model <- aov(formula, data = voting_5y_final)
  print(summary(model))
  
  # Wenn signifikant, Tukey-HSD
  p_value <- summary(model)[[1]][["Pr(>F)"]][1]
  if (!is.na(p_value) && p_value < 0.05) {
    cat("\n→ Signifikanter Unterschied gefunden. Tukey-HSD-Test:\n")
    print(TukeyHSD(model))
  } else {
    cat("\n→ Kein signifikanter Unterschied. Kein Tukey-Test notwendig.\n")
  }
}
```
...
# Tabelle 3: VARIANZ
df3 <- head(summary_stats_v1)
ft3 <- format_table(df3)

# Tabelle 4: VARIANZ Gruppiert nach Rechtsform
df4 <- head(summary_stats_v2)
ft4 <- format_table(df4)
...
doc <- body_add_par(doc, "") # Leerzeile
doc <- body_add_par(doc,
                    "Mittlere Abweichung der Parolen vom Volksentscheid - Mit Standardabweichung. Gruppiert nach Departement.",
                    style = "heading 1")
doc <- body_add_flextable(doc,
                          ft4)

# Speichern 
print(doc, target = "result/Annahme_Ablehnung_Varianz.docx")
```
...
HIER WEITERARBEITEN - Korrelation!!
...
voting_5y_final$annahme <- as.numeric(voting_5y_final$annahme)

# Fälle identifizieren
paradoxe_faelle <- voting_5y_final %>%
  filter(
    (annahme == 1 & (state_parole_Ja < 50 | kt_parole_Ja < 50 | gemeinde_parole_Ja < 50)) |
    (annahme == 0 & (state_parole_Ja >= 50 | kt_parole_Ja >= 50 | gemeinde_parole_Ja >= 50))
  ) %>%
...
cor.test(voting_5y_final$state_parole_Ja, voting_5y_final$annahme)
cor.test(voting_5y_final$kt_parole_Ja, voting_5y_final$annahme)
cor.test(voting_5y_final$gemeinde_parole_Ja, voting_5y_final$annahme)
...
modell <- glm(annahme ~ state_parole_Ja + kt_parole_Ja + gemeinde_parole_Ja, 
...
#Scatterplot
ggplot(voting_5y_final, aes(x=state_parole_Ja, y=annahme)) +
...
voting_5y_final <- voting_5y_final %>%
  mutate(across(c(inserate_total,
                  inserate_ja,
                  inserate_nein,
                  inserate_neutral,
                  mediares_tot,
                  mediaton_tot), as.numeric))
...
### Daten für Korrelationsmatrix und Regressionsmodell

```{r}

str(voting_5y_final)

# Fehelende Werte/NA zeigen
Abstract(voting_5y_final)

# Variablen "Inserate.." sowie "leer", "ungültig" und "gultig" nur bis Ende 2023 

# a) Zeitraum 2019–2025 (nicht alle Variablen über gesammten Zeitraum verfügbar)
df_2019_2025 <- voting_5y_final %>%
  filter(year >= 2019 & year <= 2025)

# b) Zeitraum 2019-2023 (alle Variablen durchgehend verfügbar)
df_2019_2023 <- voting_5y_final %>%
  filter(year >= 2019 & year <= 2023)

```

### Korrelationsmatrix (GUT!)

```{r}

####
####Interpretieren!
#### Verwendung von Volkja_proz
####

# Korrelation zwischen tatsächlichem Ergebnis und Parolen
# Korrelation berechnen

# Nur relevante Spalten auswählen
cor_data <- df_2019_2025 %>%
  select(#annahme,            # für logistische Regression
         volkja_proz,         # für Korrelationsanalysen, da kontinuierlich.
         state_parole_Ja,     # Prozente
         kt_parole_Ja,        # Prozente
         gemeinde_parole_Ja,  # Prozente
         #nrja,
         nrja_pct,
         #nrnein,
         nrnein_pct,
...
#c) Scatterplots zur Visualisierung
# Scatterplot für jede Ebene
ggplot(df_2019_2025, aes(x = state_parole_Ja, y = volkja_proz)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Tatsächliche vs. erwartete Ja-Stimmen (Staatsebene)")

ggplot(df_2019_2025, aes(x = kt_parole_Ja, y = volkja_proz)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Tatsächliche vs. erwartete Ja-Stimmen (Kantonsebene)")

ggplot(df_2019_2025, aes(x = gemeinde_parole_Ja, y = volkja_proz)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(title = "Tatsächliche vs. erwartete Ja-Stimmen (Gemeindeebene)")


# 
# Interpretation von Scatterplots mit Trendlinie
# 1. Trendlinie betrachten
# 
# Eine ansteigende Trendlinie von links unten nach rechts oben zeigt einen positiven Zusammenhang: Je mehr Parteien auf einer Ebene die Ja-Parole ausgeben, desto höher ist tendenziell der tatsächliche Ja-Stimmenanteil (volkja_proz).
# 
# Eine abfallende Trendlinie würde auf einen negativen Zusammenhang hindeuten (was hier nicht zu erwarten ist).
# 
# Ist die Trendlinie flach oder kaum erkennbar, besteht kein oder nur ein sehr schwacher Zusammenhang zwischen den Variablen.
# 
# 2. Streuung der Punkte
# 
# Liegen die Punkte nah an der Trendlinie, ist der Zusammenhang stark und die Parolen sind ein guter Prädiktor für das Abstimmungsergebnis.
# 
# Sind die Punkte weit gestreut, ist der Zusammenhang schwächer und andere Faktoren spielen eine größere Rolle.
# 
# 3. Ausreißer identifizieren
# 
# Einzelne Punkte, die weit von der Trendlinie entfernt liegen, sind Ausreißer. Sie können auf spezielle Abstimmungen oder außergewöhnliche Umstände hinweisen und sollten ggf. genauer untersucht werden.
# 
# 4. Quadrantenanalyse (optional)
# 
# Du kannst das Scatterplot in vier Quadranten einteilen, um Cluster zu identifizieren, z.B. Abstimmungen mit wenig Parolenunterstützung und wenig Ja-Stimmen (links unten) oder viel Parolenunterstützung und viele Ja-Stimmen (rechts oben).
# 
# Was bedeutet das für deine Analyse?
# Eine deutliche, ansteigende Trendlinie in deinen Scatterplots bestätigt, dass die Parolen (egal ob auf Staats-, Kantons- oder Gemeindeebene) einen positiven Einfluss auf das tatsächliche Abstimmungsergebnis haben.
# 
# Die Steilheit der Trendlinie und die Nähe der Punkte zur Linie geben Aufschluss über die Stärke dieses Einflusses: Je steiler und enger, desto stärker der Zusammenhang.
# 
# Die Scatterplots helfen dir, schnell zu erkennen, ob und wie stark die Parolen mit dem Abstimmungsergebnis zusammenhängen und ob es Ausreißer gibt, die weiter untersucht werden sollten.
# 
# Zusammengefasst:
# Die Scatterplots visualisieren, wie stark und in welche Richtung die Parolen auf den verschiedenen Ebenen mit dem tatsächlichen Ja-Stimmenanteil zusammenhängen. Eine klare, ansteigende Trendlinie und wenig Streuung bedeuten, dass die Parolen ein guter Prädiktor für das Ergebnis sind. Ausreißer oder starke Streuung deuten darauf hin, dass weitere Faktoren das Abstimmungsergebnis beeinflussen können.
...
# 4. Erweiterte Analyse: Einfluss weiterer Variablen
# a) Korrelationsmatrix mit weiteren Variablen


# Relevante Variablen auswählen
cor_data_extended <- df_2019_2025 %>%
  select(volkja_proz, state_parole_Ja, kt_parole_Ja, gemeinde_parole_Ja,
         dep, rechtsform, bet, inserate_total, inserate_ja, inserate_nein,
         inserate_neutral, mediares_tot, mediaton_tot)

str(cor_data_extended)
# Nur numerische Variablen für die Korrelation
cor_data_numeric <- cor_data_extended %>% select(where(is.numeric))
cor_matrix_ext <- cor(cor_data_numeric, use = "pairwise.complete.obs")
str(cor_data_numeric)
corrplot(cor_matrix_ext, method = "color", type = "upper")


#b) Paarweise Scatterplots (optional, für mehr Übersicht)
# Nur ein paar ausgewählte Variablen für Übersichtlichkeit
GGally::ggpairs(cor_data_numeric[, 1:10])
...
-   Die lineare Regression zeigt feine Unterschiede im Ja-Anteil.

-   Die logistische Regression zeigt, was wirklich den Ausschlag für Annahme/Ablehnung gibt.

#### Regresssion - Lineares Modell

```{r}

# Zielvariable: "volkja_proz" (Prozentsatz der Ja-Stimmen, metrisch)
# 
# Regressionstyp: Lineare Regression (lm)
# 
# Vorteile:
# - volle Information (wie hoch ist die Zustimmung in Prozent?).
# - auch kleine Unterschiede zwischen den Abstimmungen analysieren.
# 
# Fragestellung:
# - Wie stark beeinflussen Inserate, Medienresonanz, etc. den Ja-Anteil?
# 
# Interpretation:
# - Effekt zeigt, wie viele Prozentpunkte sich die Ja-Stimmen bei Änderung der Prädiktoren verändern. e.g. "Ein Anstieg der Ja-Inserate um 10% erhöht den Ja-Stimmen-ANTEIL um 1,5 Prozentpunkte."
  ...
  names(voting_5y_final)
  str(voting_5y_final)
  
  # Zielvariable und auszuschließende Variablen
  target <- "volkja_proz"
  exclude <- c("annahme", "titel_kurz_d", "anr", "year")
  
  # Prädiktoren bestimmen
  predictors <- setdiff(names(voting_5y_final), c(target, exclude))
  
  # Formel als Text zusammenbauen
  formel_text <- paste(target, "~", paste(predictors, collapse = " + "))
  cat(formel_text)
  ##############################################
...
##############################################
# Prädiktorenauswahl aüf Lineares Modell
# Alle Variablen ausser
# - Zielvariable (volkja_proz) und
# - nicht-numerische ("annahme" & "titel")

full_model_lm <- lm(volkja_proz ~ . - annahme - titel_kurz_d,
                    data = voting_5y_final)


step_model_lm <- step(full_model_lm, direction = "both")

summary(step_model_lm)


par(mfrow = c(2, 2))   # 2x2-Layout für vier Plots
plot(step_model_lm)
par(mfrow = c(1, 1))

library(effects)
plot(allEffects(step_model_lm))

plot(voting_5y_final$mediares_tot, voting_5y_final$volkja_proz,
     xlab = "Medienresonanz", ylab = "Ja-Stimmen-Anteil (%)")
abline(lm(volkja_proz ~ mediares_tot, data = voting_5y_final), col = "red")




# Lineares Regressionsmodell mit volkja_proz als Zielvariable
model_lm <- lm(volkja_proz ~ inserate_ja_pct +
                 inserate_nein_pct +
                 bet,
               data = voting_5y_final)
summary(model_lm)
...
#### Regression - logistisches Modell

```{r}

# Zielvariable: "annahme" (Ja/Nein, binär: 1 = angenommen, 0 = abgelehnt)
# Regressionstyp: Logistische Regression (glm(..., family = binomial))
# 
# Vorteile:
# - welche Faktoren beinflussen die Wahrscheinlichkeit einer Annahme
# - primär um zu schauen ob eine Vorlage angenommen/abgelehnt wurde
# 
#  Fragestellung:
# - Welche Faktoren erhöhen die Chance, dass eine Vorlage angenommen wird?
# 
# Interpretation:
# - Die Effekte zeigen, wie sich die Wahrscheinlichkeit der Annahme verändert. e.g. "Ein Anstieg der Ja-Inserate um 10% erhöht die WAHRSCHEINLICHKEIT einer Annahme um 20%."



model_logit <- glm(annahme ~ inserate_ja_pct +
                     inserate_nein_pct +
                     bet,
                   data = voting_5y_final,
                   family = binomial)
...
# Wähle alle Variablen aus, von denen du erwartest, dass sie einen Einfluss auf deine Zielgröße haben könnten.
# 
# Prüfe nach der Regression, welche Prädiktoren tatsächlich signifikant sind (über die p-Werte im Output).
# 
# Entferne ggf. nicht-signifikante Prädiktoren, um das Modell zu vereinfachen (Prinzip der Sparsamkeit).
```
